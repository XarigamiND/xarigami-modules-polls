...
function polls_admin_list()
{
    ...
    foreach ($polls as $poll) {
        ...
        if (($poll['state'] == 'open' ||  $poll['state'] == 'upcoming') && $poll['open']) {
            ...
            $row['action_close'] = xarModURL('polls', 'admin', 'close',
                                array('pid' => $poll['pid'], 'authid' => $authid, 'status' => $status)
            );

            $row['action_modify'] = xarModURL('polls', 'admin', 'modify',
                                    array('pid' => $poll['pid'])
            );
        }
        ...
        if ($poll['state'] == 'closed' || !$poll['open']) {
            ...
            $row['action_open'] = xarModURL('polls', 'admin', 'open',
                array('pid' => $poll['pid'], 'authid' => $authid, 'status' => $status)
            );
        }
