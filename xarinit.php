...
function polls_upgrade($oldversion)
{
    ...
    switch($oldversion) {
        ...
        case '1.5.0':
        case '1.6.0':
        //upgrade to signify code changes for xarigami 1.4.0
            //removal or references and updates for xarError*
            //add pollintro text field
            $dbconn = xarDBGetConn();
            $xartable = xarDBGetTables();
            $pollstable = $xartable['polls'];
             xarDBLoadTableMaintenanceAPI();
            ...
            $result = $dbconn->Execute($query);
            if (!$result) return;
        case '1.6.1': //current version
            //signify some fixes in code
